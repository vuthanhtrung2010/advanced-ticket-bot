...
client.on("interactionCreate", async (btn) => {
  if (btn.values == "stop_client") {
    ...
    try {
      btn.reply({
        content: "<a:Loading:959804807529513000> **Shutting down the bot...**",
        ephemeral: true
      })
      setTimeout(() => {
        process.exit()
  ...
  if (btn.values == "rename_client") {
    ...
    btn.reply({ content: "<a:Loading:959804807529513000> **Send new Bot Name**",
        ephemeral: true})
  ...
  if (btn.values == "changeav_client") {
    ...
    btn.reply({ content: "<a:Loading:959804807529513000> **Send the New Bot Image**",
        ephemeral: true})
    collector.on("collect", async (msg) => {
        let url = msg.content;
      if(msg.content.includes(`https://`)) {
        btn.editReply({ content: "<a:Loading:959804807529513000> **Changing avatar...**",
        ephemeral: true})
